...
def sampler(samples, nModels, x, ydata):
    ...
    [likelihood_current, fx] = likelihood_func(nModels, x, ydata, mu_current, sig_current, w_current, tau_current)
    ...
    for i in range(samples-1):
       ...
       # print  likelihood_current, mu_current, nu_current,  eta_current , 'current' 

        if nModels ==1:
           weights =  [1]

        if nModels ==2:
           weights =  np.random.dirichlet((1,1), 1)  # (genreate vector that  adds to 1)
        if nModels ==3:
           weights =  np.random.dirichlet((1,1,1), 1)  # (vector adds to 1)
        if nModels ==4:
           weights =  np.random.dirichlet((1,1,1,1), 1)  # (vector adds to 1)
        if nModels ==5:
           weights =  np.random.dirichlet((1,1,1,1,1), 1)  # (vector adds to 1)
        ...
        for j in range(nModels):
            if mu_proposal[j] < 0 or mu_proposal[j] > 1 : #ensure they stay between a range
        ...
        [likelihood_proposal, fx] = likelihood_func(nModels, x, ydata,  mu_proposal, sig_proposal, w_proposal, tau_proposal)
        ...
        if u < mh_prob:
            ...
            print  likelihood_current, mu_current, nu_current,  eta_current , 'accepted' 
...
def main():
	...
	[pos_mu, pos_sig, pos_w, pos_tau, fx_samples] = sampler(NumSamples, nModels, x, ydata)
